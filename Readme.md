...
dotnet run --project src/Runner/Runner.csproj После запуска в консоли появится меню со списком всех найденных решений, сгруппированных по задачам. Вы можете выбрать номер решения и нажать Enter для его выполнения.
...
1.  **Создайте проект**: В терминале, из корневой папки, выполните:
    dotnet new classlib -n Tasks.StringReversal -o src/Tasks/StringReversal2.  **Добавьте в решение и настройте ссылки**:
